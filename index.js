...
// 🏡 Task 2: Harder Math
/* Create your calculator! Use the formula in the ReadMe to run calculations on 
your numbers. Save the final value into a variable called monthlyRate.
...
var M = amountBarrowed * (numerator / denominator);
...
function mortgageCalculator() 
{
    var numerator = monthlyInterestRate * (Math.pow( (1 + monthlyInterestRate), (periods) ) );
    var denominator = (Math.pow( (1 + monthlyInterestRate), (periods) ) ) - 1;
    var M = principal * (numerator / denominator);
...
function variableInterestRate(principal, interestRate, years)
{
    ...
    console.log(name + ", with an interest rate of " + interestRate + ", your monthly rate is " + Math.round(M));
    for (let i = 0; i < 10; i++)
    {
      interestRate += .005;
      let monthlyInterestRate = interestRate / 12;
